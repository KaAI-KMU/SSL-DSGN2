...
__global__ void BuildCostVolumeForward(const int nthreads, 
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    {
        const T* offset_right = right + (n * channels + c) * img_height * img_width;
...
__global__ void BuildCostVolumeBackwardFeature(const int nthreads, 
    ...
    const int num_batch, const int channels, const int height,
    const int width, const int max_disp,
    T* grad_left, T* grad_right, int downsample) {
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    int pw = index % width;
    int ph = (index / width) % height;
    int pd = (index / width / height) % max_disp;
    ...
    // left
    atomicAdd(grad_left + ((n * channels + c) * img_height + ih) * img_width + iw, static_cast<T>(grad[index_L]));
    ...
    if (iw + shift_pd >= 0. && iw + shift_pd <= img_width - 1)
    {
        ...
        T* offset_grad_right = grad_right + (n * channels + c) * img_height * img_width;
        if (w1 >= 1e-10)
