...
class SECONDHead(RoIHeadTemplate):
    ...
    def roi_grid_pool(self, batch_dict): ##test
        """
        Args:
            batch_dict:
                batch_size:
    ...
    def get_loss(self, tb_dict=None):
        ...
        rcnn_acc_cls = calculate_accuracy(self.forward_ret_dict)
        rcnn_acc += rcnn_acc_cls

        tb_dict['rcnn_acc'] = rcnn_acc.item()
...
def calculate_accuracy(forward_ret_dict):
    ...
    # Mean Squared Error
    se = (rcnn_iou_flat - rcnn_iou_labels) ** 2
    mse = se.sum() / len(rcnn_iou_flat)
    mean = rcnn_iou_flat.sum() / len(rcnn_iou_flat)
    var = ((rcnn_iou_flat - mean) ** 2).sum() / (len(rcnn_iou_flat) - 1)
    acc = mse/var
